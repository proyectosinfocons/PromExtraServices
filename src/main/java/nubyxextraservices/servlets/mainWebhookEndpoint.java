 ...
 * @author Luis D
 */
public class mainWebhookEndpoint extends HttpServlet {

    private final String SERVLET = "mainWebhookEndpoint";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        JSONObject requestBody = ServicioWeb.leerJsonDelRequest(request);
        Log.info(LOG_HEADER + "INPUT: body = " + requestBody.toString(), SERVLET);
        JSONObject objectAddress = requestBody.getJSONObject("subscriber");
        String phoneAddress=objectAddress.getString("identifier");
        PrintWriter out = response.getWriter();
        response.setStatus(200);
        out.flush();

        ProcessingThread processingThread = new ProcessingThread(requestBody, phoneAddress, LOG_HEADER, SERVLET);
